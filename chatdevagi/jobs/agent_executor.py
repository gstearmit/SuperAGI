...
from chatdevagi.agent.types.wait_step_status import AgentWorkflowStepWaitStatus
...
from chatdevagi.llms.llm_model_factory import get_model
...
from chatdevagi.models.workflows.agent_workflow_step_wait import AgentWorkflowStepWait
...
from chatdevagi.agent.types.agent_workflow_step_action_types import AgentWorkflowStepAction
from chatdevagi.agent.types.agent_execution_status import AgentExecutionStatus

# from chatdevagi.helper.tool_helper import get_tool_config_by_key
    ...
    def execute_next_step(self, agent_execution_id):
        ...
        try:
            ...
            try:
                ...
                chatdevagi.worker.execute_agent.apply_async((agent_execution_id, datetime.now()), countdown=15)
                return
            ...
            if agent_execution.status == "COMPLETED" or agent_execution.status == "WAITING_FOR_PERMISSION":
                ...
                session.close()
                return
            chatdevagi.worker.execute_agent.apply_async((agent_execution_id, datetime.now()), countdown=2)
            # chatdevagi.worker.execute_agent.delay(agent_execution_id, datetime.now())
        finally:
            session.close()
            engine.dispose()
