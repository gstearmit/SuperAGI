...
from chatdevagi.tools.base_tool import BaseTool, ToolConfiguration
from chatdevagi.tools.base_tool import BaseToolkit
    ...
    for folder_path in folder_paths:
        ...
        for folder_name in os.listdir(folder_path):
            ...
            if not os.path.isdir(folder_dir):
                continue
                # sys.path.append(os.path.abspath('chatdevagi/tools/email'))
...
def register_toolkits(session, organisation):
    tool_paths = ["chatdevagi/tools", "chatdevagi/tools/external_tools"]
    # if get_config("ENV", "DEV") == "PROD":
    #     tool_paths.append("chatdevagi/tools/marketplace_tools")
...
def register_marketplace_toolkits(session, organisation):
    tool_paths = ["chatdevagi/tools", "chatdevagi/tools/external_tools","chatdevagi/tools/marketplace_tools"]
    if organisation is not None:
        process_files(tool_paths, session, organisation)
...
def handle_tools_import():
    ...
    tool_paths = ["chatdevagi/tools", "chatdevagi/tools/marketplace_tools", "chatdevagi/tools/external_tools"]
