...
from chatdevagi.llms.llm_model_factory import get_model, build_model_with_api_key
...
def test_build_model_with_openai(mock_openai, monkeypatch):
    monkeypatch.setattr('chatdevagi.llms.llm_model_factory.OpenAi', mock_openai)
...
def test_build_model_with_replicate(mock_replicate, monkeypatch):
    monkeypatch.setattr('chatdevagi.llms.llm_model_factory.Replicate', mock_replicate)
...
def test_build_model_with_openai(mock_openai, monkeypatch):
    monkeypatch.setattr('chatdevagi.llms.llm_model_factory.OpenAi', mock_openai)  # Replace 'your_module' with the actual module name
    model = build_model_with_api_key('OpenAi', 'fake_key')
    mock_openai.assert_called_once_with(api_key='fake_key')
    assert isinstance(model, Mock)

def test_build_model_with_replicate(mock_replicate, monkeypatch):
    monkeypatch.setattr('chatdevagi.llms.llm_model_factory.Replicate', mock_replicate)  # Replace 'your_module' with the actual module name
    model = build_model_with_api_key('Replicate', 'fake_key')
    mock_replicate.assert_called_once_with(api_key='fake_key')
    assert isinstance(model, Mock)

def test_build_model_with_google_palm(mock_google_palm, monkeypatch):
    monkeypatch.setattr('chatdevagi.llms.llm_model_factory.GooglePalm', mock_google_palm)  # Replace 'your_module' with the actual module name
...
def test_build_model_with_hugging_face(mock_hugging_face, monkeypatch):
    monkeypatch.setattr('chatdevagi.llms.llm_model_factory.HuggingFace', mock_hugging_face)  # Replace 'your_module' with the actual module name
