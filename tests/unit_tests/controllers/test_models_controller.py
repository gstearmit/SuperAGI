...
from chatdevagi.helper.llm_loader import LLMLoader

client = TestClient(app)

@patch('chatdevagi.controllers.models_controller.db')
...
@patch('chatdevagi.controllers.models_controller.db')
@patch('chatdevagi.controllers.models_controller.ModelsConfig.fetch_api_key', return_value = {})
...
def test_verify_end_point_success(mock_get_db):
    with patch('chatdevagi.helper.auth.db') as mock_auth_db:
...
def test_get_marketplace_models_list_success(mock_get_db):
    with patch('chatdevagi.helper.auth.get_user_organisation') as mock_get_user_org, \
        patch('chatdevagi.helper.auth.db') as mock_auth_db, \
        patch('chatdevagi.controllers.models_controller.requests.get') as mock_get:
