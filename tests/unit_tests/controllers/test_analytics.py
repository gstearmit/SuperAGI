...
@patch('chatdevagi.controllers.analytics.db')
def test_get_metrics_success(mock_get_db):
    with patch('chatdevagi.helper.auth.get_user_organisation') as mock_get_user_org, \
                patch('chatdevagi.controllers.analytics.db') as mock_db, \
                patch('chatdevagi.controllers.analytics.AnalyticsHelper') as mock_helper, \
...
def test_get_tools_user_success(mock_get_db):
    with patch('chatdevagi.helper.auth.get_user_organisation') as mock_get_user_org, \
                    patch('chatdevagi.controllers.analytics.db') as mock_db, \
                    patch('chatdevagi.controllers.analytics.ToolsHandler') as mock_handler, \
                    patch('chatdevagi.helper.auth.db') as mock_auth_db:
