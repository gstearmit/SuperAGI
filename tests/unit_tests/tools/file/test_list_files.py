...
def test_execute(list_file_tool):
    mock_get_formatted_agent_level_path = MagicMock(return_value="chatdevagi/workspace/input/{agent_id}/")

    with patch.object(ListFileTool, 'list_files', return_value=['file1.txt', 'file2.txt']), \
         patch('chatdevagi.helper.resource_helper.ResourceHelper.get_formatted_agent_level_path', new=mock_get_formatted_agent_level_path):
