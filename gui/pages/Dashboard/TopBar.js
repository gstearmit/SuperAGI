...
export default function TopBar({selectedProject, userName, env}) {
  ...
  const logoutUser = () => {
    ...
    Cookies.set('accessToken', '', { expires: new Date(0),domain: '.chatdevagi.com', path: '/'});
    Cookies.set('Source', 'app.chatdevagi', {domain: '.chatdevagi.com', path: '/'});
    Cookies.set('mixpanel_initialized', 'false', {domain: '.chatdevagi.com', path: '/'});
    refreshUrl();
    router.reload();
  };
